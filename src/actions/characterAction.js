...
export const characterSearchFetch = (charName) => {
  ...
  // 캐릭터 검색 이력 저장
  let characterSearchHistory = window.localStorage.getItem(
  ...
  if (characterSearchHistory !== null) {
    ...
    if (histories[0] !== charName) {
      ...
      if (histories.length > 4) {
        ...
        characterSearchHistory = characterSearchHistory.substring(
          0,
          lastCommaIndex
        );
      }
  ...
  return (dispatch) => {
    callAPI(
      `/servers/all/characters?characterName=${encodeURIComponent(
        charName
      )}${wordType}&limit=200&`,
      {},
      dispatch
...
export const selectCharacter = (
  ...
  let selectedCharacters = Object.assign(
    [],
    ...
    if (isSelect) {
      if (selectedCharacters.length >= 4) {
        ...
        if (isAddPossible) {
          selectedCharacters = [
            ...selectedCharacters,
            { serverId, characterId, characterName, timeline: [] },
          ];
          window.localStorage.setItem(
            "selectedCharacters",
    ...
    callAPI(
      ...
      if (status === 200) {
        ...
        data.equipment.forEach((eq) => {
          // 장착아이템 총 성장레벨 계산
          eq.growInfo &&
            eq.growInfo.options.forEach((option) => {
          ...
          filteredItems = [
            ...filteredItems,
            ...allItems
              .filter(({ itemName }) => itemName === eq.itemName)
              .map((item) => {
                // 장착 장비 TAG요약 정보 획득
                item.tags[0]
                  .split(",")
        ...
        // map to array
        tagEquipmentSummary = Array.from(
          tagEquipmentSummary,
          ([name, value]) => ({ name, value })
        );
        // 장착 장비 TAG요약 정보 정렬(sort)
        tagEquipmentSummary.sort(
          ({ value }, { value: bValue }) => bValue - value
        );
...
export const characterTimelineFetchNext = async (
  ...
  callAPI(
    `/servers/${serverId}/characters/${characterId}/timeline?next=${next}&`,
    {},
    dispatch
  ).then((response) => {
    const { status, data } = response;
